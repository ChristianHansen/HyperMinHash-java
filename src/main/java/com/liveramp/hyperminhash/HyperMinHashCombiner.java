  ...
  public HyperMinHash union(HyperMinHash... sketches) {
    ...
    for (int i = 0; i < numRegisters; i++) {
      for (HyperMinHash sketch : sketches) {
        int currentExponent = LongPacker.unpackExponent(mergedSketch.packedRegisters[i], q, r);
        int incomingExponent = LongPacker.unpackExponent(sketch.packedRegisters[i], q, r);
